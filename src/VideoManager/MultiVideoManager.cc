...
void MultiVideoManager::_setupReceiver(QGCToolbox *toolbox, unsigned int id)
{
    ...
    connect(_videoReceiver[id], &VideoReceiver::decodingChanged, this, [this, id](bool active)
        ...
        case 2:
            ...
            break;
        case 3:
            emit _videoManager->decodingChanged3();
            break;
        case 4:
            emit _videoManager->decodingChanged4();
            break;
        case 5:
            emit _videoManager->decodingChanged5();
            break;
        } });
}
...
void MultiVideoManager::setToolbox(QGCToolbox *toolbox)
{
    ...
    connect(_videoSettings->udpPort3(), &Fact::rawValueChanged, this, &MultiVideoManager::_udpPortChanged);
    connect(_videoSettings->udpPort4(), &Fact::rawValueChanged, this, &MultiVideoManager::_udpPortChanged);
    connect(_videoSettings->udpPort5(), &Fact::rawValueChanged, this, &MultiVideoManager::_udpPortChanged);
...
{
    // Xem lai do minh ko dung MultiVideoWindow.qml
    // (https://github.com/DeepwaterExploration/qgroundcontrol/blob/v1.0.0-dwe-release/src/ui/MultiVideoWindow.qml)
    // QQuickWindow *root = qgcApp()->multiVideoWindow();  // qgcApp() in file QGCApplication.h
    ...
    _updateVideoURI(3, _videoSettings->udpPort3()->rawValue().toInt());
    _updateVideoURI(4, _videoSettings->udpPort4()->rawValue().toInt());
    _updateVideoURI(5, _videoSettings->udpPort5()->rawValue().toInt());
